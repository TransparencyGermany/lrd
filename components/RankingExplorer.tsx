...
function buildEmbedSnippet(year: Year): string {
  return `<iframe src="https://www.lobbyranking.de/embed?year=${year}" width="800" height="640" frameborder="0"></iframe>`;
}
...
export default function RankingExplorer({ views, year, onYearChange }: RankingExplorerProps) {
  ...
  function copyEmbed() {
    const code = buildEmbedSnippet(year);
  ...
  return (
    <div className={styles.explorer}>
      ...
      <div className={styles.actions}>
        ...
        <button type="button" className={styles.actionButton} onClick={copyEmbed}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <rect x="3" y="6" width="18" height="12" rx="2" />
            <path d="M8 10v4M16 10v4" />
          </svg>
          {embedCopied ? "Code kopiert!" : "Als iFrame einbetten"}
        </button>
      </div>
    </div>
  );
